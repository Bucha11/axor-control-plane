...
| M6 | done | `vault_creds` (dispense/scope/fail-closed/rotate/revoke-narrowing) + `vault_signing` (sign-not-surrender, per-request auth, audit) + wall (separate tokens, AST import test); Settings two panes. Sink-side injection wired in `axor_proxy.vault`: opt-in per tool (`AXOR_VAULT_TOOLS`), fetched at call time, no cache, fail-closed, injection recorded in the trace without key material. Dev backend only — decision #13's Vault/KMS-class store is not written, and boot says so |
