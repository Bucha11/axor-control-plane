...
| P | Item | Done-when | Status |
|---|---|---|---|
| P0 | **Publish to PyPI**: `axor-proxy` (uvx path is quoted all over the UI/docs and currently 404s), `axor-backend`. Compose images to GHCR. | `uvx axor-proxy --demo` works on a clean machine. | ◐ (build+install+CLI verified clean-room: both wheels build, install against PyPI-resolved core 0.9.2 / eval 0.1.0, `--demo` serves healthz; release.yml + **docs/RELEASING.md** ready. Remaining is human-only: register the two trusted publishers on pypi.org + push tag v0.1.0.) |
