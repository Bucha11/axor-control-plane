...
Consolidated draft. Extends ui-spec v0.13 — **the single-agent layer is implemented and in production**; this document specifies the multi-agent layer built *on top of* it, plus the wire changes it requires. v2 does not edit v0.13 or its code: it is an additive layer. Every compatibility claim below is therefore a requirement against shipped code, not a design aspiration.
...
| v2-18 | Demo composition (Ch.6) | Two-tree containment is the multi-agent hero; single-agent split demoted. Amend v0.13 §7 at reference-time. |
...
## Implementation tasks (additive layer over shipped v1)
v1 is in production; v2 adds a layer, it does not rewrite v1.
1. Generalize the runtime (Ch.4) — labels in envelopes, boundary gates both ends; single-node path byte-identical (regression gate).
2. Extend EvidenceCase (Ch.3) — optional causal_subgraph, derived on open; renderer branches on subgraph size.
3. Add measurement axes (Ch.2) — containment + systemic label alongside §7 integrity.
4. Inter-federation (Ch.1) — peer keysets, trust levels, attestation; greenfield.
5. Federation vault (Ch.5) — federation-scoped tool creds + signing custody, walled.
6. Protocol → v0.2 (Ch.6) — JCS + context_excision, additive.

Doc-hygiene (not code): v2 supersedes v0.13 §14.1 (A2A), generalizes §7/§8, extends §14.2 (vault) — noted for the reader, not edited into the shipped spec.
