...
event bus, the notifier, the operator keyring, the subgraph cache and the
resolved config. Handlers used to reach for them through
``request.app.state.x``, which meant every route signature carried a ``Request``
it did not otherwise want, and no route declared what it actually needed.
...
from axor_backend.auth import Principal
