...
Scope is ``(run_id, ref)``. ``covers`` names value refs, and a value ref means
something only inside the run that minted it (see :mod:`provenance`) — an
attestation scoped to the bare ref covered the same ref in every other run,
which is precisely the operator-side reputation-laundering channel Sentinel's
attestation module exists in order not to have.
...
def record_from_fact(fact: dict[str, Any], ref: str) -> AttestationRecord:
    """One fact, scoped to one branch it covers.
...
def covering(facts: list[dict[str, Any]], ref: str) -> list[dict[str, Any]]:
    ...
    returns them). Nothing is filtered out — history runs in both directions, so
    a revoked attestation and the revocation that ended it both stay — but each
    entry says whether its coverage is still in effect.

    A revocation is matched by the ``fact_id`` it names, not by its own
    ``covers``: a revocation that forgot to repeat the branch would otherwise be
    a silent no-op, recorded and ignored.
    """
    attesting = {
