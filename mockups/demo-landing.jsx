...
const C = {
  bg: "#0E1216", panel: "#191F26", line: "#262E37",
...
const path = (a, b) => `M ${N[a].x} ${N[a].y} L ${N[b].x} ${N[b].y}`;
...
export default function App() {
  ...
  const done = (k) => step >= 0 && SCRIPT.slice(0, step + 1).some((s) => s[k]);
  const cur = step >= 0 ? SCRIPT[step] : {};
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "Inter, system-ui, sans-serif", display: "flex", flexDirection: "column", alignItems: "center", padding: "48px 20px" }}>
      <style>{`
      ...
      <div style={{ maxWidth: 680, width: "100%" }}>
        ...
        {/* the stage */}
        <div style={{ background: "#11151A", border: `1px solid ${C.line}`, borderRadius: 10, position: "relative", overflow: "hidden" }}>
          <svg viewBox="0 0 660 300" style={{ width: "100%", display: "block" }}>
            {/* static edges */}
            {[["user","agent"],["agent","email"],["agent","search"],["agent","slack"]].map(([a,b]) => (
            ...
            {playing && cur.pulse && !receipt && (
              <circle key={step} r="5" fill={cur.color} style={{ filter: `drop-shadow(0 0 5px ${cur.color})` }}>
        ...
        {receipt && (
          <div style={{ animation: "fadeUp .5s ease both", marginTop: 16, background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, overflow: "hidden" }}>
            ...
            <div style={{ borderTop: `1px solid ${C.line}`, padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(229,72,77,0.05)" }}>
              ...
              <button style={{ display: "flex", alignItems: "center", gap: 8, background: C.green, border: "none", borderRadius: 5, color: C.bg, fontFamily: MONO, fontSize: 12.5, fontWeight: 700, padding: "8px 16px", cursor: "pointer" }}>
